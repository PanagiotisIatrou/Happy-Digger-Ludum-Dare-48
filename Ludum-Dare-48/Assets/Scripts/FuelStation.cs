...
{
    public TextMeshProUGUI FuelStationsText;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            isInside = true;
            FuelStationsText.gameObject.SetActive(true);
        }
    }
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        if (collision.name == "Player")
        {
            float fuelToBuy = playerFuel.GetMissingFuel();
            int cost = Mathf.CeilToInt(fuelToBuy);
            FuelStationsText.SetText("PRESS F TO REFUEL\nCOST: $" + cost);
        }
    }
    ...
    private void OnTriggerExit2D(Collider2D collision)
    {
        ...
        {
            isInside = false;
            FuelStationsText.gameObject.SetActive(false);
        }
    }
}
